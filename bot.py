...
URGENCY = {"no urgency": 0, "urgent (sfw)": 1150, "urgent (nsfw)": 2300}
...
TEXTS = {
    "en": {
        ...
        "step_titles": {
            ...
            "urgency": "⚡ *need it urgently?*\n_urgency is billed separately after your main payment._",
            "minutes": "⏱️ *how many minutes?*\n_select duration for your session._",
        },
        ...
        "note": "note",
        "total": "⭐ *total:* {:,} stars",
        "split_note": "_split into {} payments of max {:,} stars each_",
        "urgency_note": "⚡ *urgency:* {:,} stars _(2nd step after main payment)_",
    ...
    "tr": {
        ...
        "step_titles": {
            ...
            "urgency": "⚡ *acil mi?*\n_aciliyet ücreti ana ödemeden sonra ayrıca tahsil edilir._",
            "minutes": "⏱️ *kaç dakika?*\n_oturum süresini seç._",
        },
        ...
        "note": "not",
        "total": "⭐ *toplam:* {:,} yıldız",
        "split_note": "_en fazla {:,} yıldızlık {} ödemeye bölündü_",
        "urgency_note": "⚡ *aciliyet:* {:,} yıldız _(ana ödemeden sonra 2. adım)_",
...
def calculate_stars(selections, minutes=1):
    ...
    base_stars = max(1, round(base * multiplier * minutes * USD_TO_STARS))
    urgency_stars = URGENCY.get(selections.get("urgency", "no urgency"), 0)
    return base_stars + urgency_stars
...
def build_keyboard(step, prefix, lang):
    ...
    for label in STEP_DATA[step]:
        translated = translate_label(lang, label)
        d = desc(lang, step, label)
        btn_text = f"{translated}  —  {d}" if d else translated
        buttons.append([InlineKeyboardButton(btn_text, callback_data=f"{prefix}|{step}|{label}")])
    return InlineKeyboardMarkup(buttons)

def build_minutes_keyboard(prefix, lang):
    label = "dakika" if lang == "tr" else "min"
    return InlineKeyboardMarkup([
        [InlineKeyboardButton(f"{i} {label}", callback_data=f"{prefix}|minutes|{i}")]
        for i in range(1, 11)
    ])
    ...
    if next_index < len(STEPS):
        ...
        if selections.get("format") in MINUTE_FORMATS:
            ...
            await query.edit_message_text(
                ...
                reply_markup=build_minutes_keyboard(prefix, lang),
                parse_mode="Markdown"
            )
        else:
            context.user_data["step"] = "note"
